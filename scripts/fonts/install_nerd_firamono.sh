...
# Install FiraMono Nerd Font using the generic font installer
FIRAMONO_FONT_URL="https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/FiraMono.zip"
FIRAMONO_FONT_FOLDER="FiraMono"
install_font_zip_to_dir "$FIRAMONO_FONT_URL" "$FONT_BASEDIR" "$FIRAMONO_FONT_FOLDER"

if command -v print_completion_message &>/dev/null; then
    print_completion_message "$env_title" "$env_verb" \
        "FiraMono Nerd Font" "fc-list | grep -i 'FiraMono' | head -n1"
else
    echo "FiraMono Nerd Font installation complete."
    fc-list | grep -i 'FiraMono' | head -n1
fi
